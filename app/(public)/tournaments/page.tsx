...
} from "@/components/arena/tournament-card";
...
export default function TournamentsPage() {
  ...
  const tournaments = useMemo(
    () =>
      ((data?.tournaments ?? []) as TournamentCardData[]).filter((t) => {
        const queryMatches = t.name
          .toLowerCase()
          .includes(search.toLowerCase());
        if (!queryMatches) return false;
        if (filter === "eFootball")
          return (t.gameId ?? "efootball") === "efootball";
        if (filter === "VALORANT") return t.gameId === "valorant";
        if (filter === "Live") return t.status === "Ongoing";
        if (filter === "Registration Open")
  ...
  return (
    <div className="mx-auto min-h-[70vh] max-w-7xl px-4 py-14 sm:px-6">
      <div className="grid gap-8 border-b border-border pb-10 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          ...
          <p className="mt-4 max-w-2xl text-muted-foreground">
            Live brackets, open registrations, and upcoming events across every
            D-One Arena game.
          </p>
        </div>
      ...
      <div className="my-8 flex items-center gap-3 overflow-x-auto pb-2">
      ...
      {data === undefined ? (
        ...
        <div className="rounded-2xl border border-dashed border-border py-20 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Try a different game, status, or search.
          </p>
        </div>
      )}
    </div>
